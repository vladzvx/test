...
$$
    begin
        RETURN QUERY
            select distinct source_chat,message_id,task_type,target_chat,messages.text,messages.caption,messages.media_id,messages.media_group_id,tasks.task_id, tasks.action_time,c.name from tasks inner join messages on tasks.source_message_id=messages.message_id inner join chats c on tasks.source_chat = c.chat_id
